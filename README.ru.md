...
Встроенно отклоняются: любой сегмент пути, начинающийся с точки, — кроме
`.well-known`, где лежат ACME-челленджи, — типы файлов, которые запрашивают
сканеры (`.php`, `.sql`, `.bak`, `.ini`, `.key` и прочие), и первые сегменты
вроде `env`, `wp-admin` или `phpmyadmin`. Короткие UUID — это 16-символьные
nanoid, поэтому пересечься с реальной подпиской они не могут, а ассеты самой
