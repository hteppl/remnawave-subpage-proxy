...
в `template` обернуть announce из панели, а не отбросить его; плейсхолдеры,
использованные самой панелью, внутри него тоже раскрываются. Если апстрим такого
заголовка не прислал, значение остается неразрешенным.
...
`traffic.force_unlimited` скрывает квоту из встроенного индикатора трафика в
приложении, независимо от того, что задано в панели:
...
Переписывается только этот заголовок. Плейсхолдеры и условия продолжают
сообщать реальную квоту, поэтому `{TRAFFIC_LIMIT}` по-прежнему даст `100.00 GB`,
а `has_traffic_limit: true` сработает. Опция управляет тем единственным, на что
шаблон повлиять не может, — встроенным индикатором трафика в приложении.
...
порядке все пользователи попадают на один сервер. `hosts.shuffle` задает группы
хостов регулярными выражениями Go, которые сопоставляются с именем сервера; при
каждом запросе хосты внутри группы перемешиваются между позициями, которые они
уже занимают, а хост, не подошедший ни под один шаблон, остается на месте:

```yaml
hosts:
  shuffle:
    - "^ru\\d+\\.example\\.com$"   # ru1, ru2, ru3 ... меняются местами
    - "^(de|nl)\\."                  # вторая, независимая группа
```

Обычная строка сопоставляется с именем сервера. Чтобы группировать по имени,
которое видит клиент — фрагмент ссылки или `ps` у vmess, `remarks` у Xray,
`tag` у sing-box, `name` прокси у Clash, — запишите mapping с ключом `name`;
если заданы оба ключа, должны совпасть оба:

```yaml
hosts:
  shuffle:
    - name: "(?i)premium"          # на каком бы сервере они ни были
    - hostname: "\\.example\\.com$"
      name: "^🇩🇪"                  # только наши немецкие хосты
```

Хост, подходящий под несколько групп, относится к первой. `".*"`
перемешивает все хосты. По умолчанию список пуст, и пока группа не задана,
тело ответа не буферизуется и не переписывается.

Формат определяется по содержимому, поэтому неважно, какой путь типа клиента
или user-agent его породил. Обычный список ссылок, в base64 или нет,
перемешивается построчно; `/json` и `/v2ray-json` переставляют целые конфиги
Xray; в конфиге sing-box перемешиваются outbound-ы узлов, а каждый selector или
urltest, перечисляющий их по тегу, переупорядочивается следом; в YAML для
Clash, Mihomo и Stash перемешивается список `proxies`, а имена в каждой
proxy-группе следуют за ним. `DIRECT`, `REJECT` и имена групп остаются на
...
отклоняются самим прокси и не доходят ни до страницы подписки, ни до панели.
В ответ уходит пустой `404`, а в лог запись попадает только на уровне `debug`:
...
любой зонд уходит апстриму как обычный запрос.

Встроенно отклоняются: любой сегмент пути, начинающийся с точки; типы файлов,
которые запрашивают сканеры (`.php`, `.sql`, `.bak`, `.ini`, `.key` и прочие),
в любом сегменте — завершающий слэш обойти проверку не позволяет; и первые
...
реальной подпиской не пересекаются, но форма нигде не проверяется: если такое
имя все же выдано, фильтр придется выключить.
