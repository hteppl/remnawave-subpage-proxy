...
хостов регулярными выражениями Go, которые сопоставляются с именем, видимым
клиенту, — фрагмент ссылки или `ps` у vmess, `remarks` у Xray, `tag` у
sing-box, `name` прокси у Clash. При каждом запросе хосты внутри группы
перемешиваются между позициями, которые они уже занимают, а хост, не подошедший
ни под один шаблон, остается на месте:
...
переименованием в панели, не меняя конфиг.
